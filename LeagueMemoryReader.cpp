...
void LeagueMemoryReader::HookToProcess() {
	...
	if (EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded)) {
		...
		for (int i = 0; i < (sizeof(hMods) / sizeof(HMODULE)); i++)
		{
			TCHAR szModName[MAX_PATH] = {};
			if (GetModuleFileNameEx(hProcess, hMods[i], szModName, MAX_PATH))
			{
				printf("%ws\n", szModName);
				HMODULE hModule = GetModuleHandle(szModName);
				MODULEINFO mi;
				GetModuleInformation(NULL, hModule, &mi, sizeof(MODULEINFO));
				moduleBaseAddr = (DWORD_PTR)mi.lpBaseOfDll;
			}
		}
...
void LeagueMemoryReader::FindHoveredObject(MemSnapshot& ms) {
	...
	int addrObjP = Mem::ReadDWORD(hProcess, moduleBaseAddr + Offsets::UnderMouseObject);
	int addrObj = Mem::ReadDWORD(hProcess, addrObjP + 0xC);
...
//std::vector<DWORD> LeagueMemoryReader::get_heroes() {
//
//	auto AIHeroClient = Mem::ReadDWORD(hProcess, moduleBaseAddr + Offsets::AIHeroClient);
//	auto pArray = Mem::ReadDWORD(hProcess, AIHeroClient + 0x4);
//	auto pList = Mem::ReadDWORD(hProcess, AIHeroClient + 0x8);
//
//	std::vector<DWORD> heroes;
//	if (pList != 0) {
//		for (int i = 0; i < pList * 4; i += 4) {
//			auto hero = Mem::ReadDWORD(hProcess, pArray + i);
//			if (hero != 0) {
...
void LeagueMemoryReader::ReadObjects(MemSnapshot& ms) {
	...
	static char buff[0x500];//500
	Mem::Read(hProcess, objectManager, buff, 0x100);//100
	...
	while (reads < maxObjects && nodesToVisit.size() > 0) {
		...
		if (netId - (unsigned long)0x40000000 > 0x1000000)
			continue;
	...
	for (int i = 0; i < nrObj; ++i) {
		...
		if (obj->networkId != 0) {
			...
			if (obj->HasUnitTags(Unit_Champion))
...
POINT LeagueMemoryReader::GetCursorPosition() {
	POINT p;
